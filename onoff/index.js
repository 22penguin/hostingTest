...
document.addEventListener("keydown", e=>{
    t.add(e.key),
    s.has(e.key) && e.preventDefault()
}
),
document.addEventListener("keyup", ({key: e})=>{
    t.delete(e)
}
);
const o = new Map
...
class m extends l {
    ...
    tick(t) {
        h() && !n() ? (this.vx = -t(this.speed),
        this.faceLeft = !0) : n() && !h() ? (this.vx = t(this.speed),
        this.faceLeft = !1) : this.vx = 0,
        this.walking = h() || n()
    }
    get faceLeft() {
...
class v extends l {
    ...
    set selected(t) {
        this._selected = Math.min(f.length - 1, Math.max(0, t || 0)),
...
const E = function t(e) {
    var s = e.split(k);
    this.frequency = t.getFrequency(s[0]) || 0,
    this.duration = t.getDuration(s[1]) || 0
};
...
D.prototype.createFxNodes = function() {
    ...
    return [["bass", 100], ["mid", 1e3], ["treble", 2500]].forEach(function(e, s) {
        (s = this[e[0]] = this.ac.createBiquadFilter()).type = "peaking",
        s.frequency.value = e[1],
        t.connect(t = s)
    }
    .bind(this)),
...
D.prototype.createOscillator = function() {
    ...
    this.customWave ? this.osc.setPeriodicWave(this.ac.createPeriodicWave.apply(this.ac, this.customWave)) : this.osc.type = this.waveType || "square",
...
D.prototype.scheduleNote = function(t, e) {
    ...
    return this.setFrequency(this.notes[t].frequency, e),
    this.smoothing && this.notes[t].frequency && this.slide(t, e, i),
    this.setFrequency(0, e + i),
    e + s
}
,
...
D.prototype.slide = function(t, e, s) {
    ...
    return this.setFrequency(this.notes[t].frequency, e + h),
    this.rampFrequency(i.frequency, e + s),
    this
}
,
D.prototype.setFrequency = function(t, e) {
    return this.osc.frequency.setValueAtTime(t, e),
    this
}
,
D.prototype.rampFrequency = function(t, e) {
    return this.osc.frequency.linearRampToValueAtTime(t, e),
    this
}
,
D.prototype.play = function(t) {
    return t = "number" == typeof t ? t : this.ac.currentTime,
    this.createOscillator(),
    this.osc.start(t),
    this.notes.forEach(function(e, s) {
...
D.prototype.stop = function() {
    return this.osc && (this.osc.onended = null,
    this.osc.disconnect(),
    this.osc = null),
    this
}
...
setInterval(function() {
    ...
    "down" === U && (N -= 1) < 1 && (U = "up",
    N += 2),
    F.gain.gain.value = .01 * N,
    S.gain.gain.value = .01 * N
}, 300);
const I = ()=>{
    L.play(),
    M.play(),
...
class K extends l {
    ...
    set value(t) {
        ...
        for (let t of this.value.toString()) {
            ...
            s.element.setAttribute("fill", `url(#n${t})`),
            s.width = 10,
            s.height = 16,
            s.x = 12 * e++,
            this.append(s)
...
class j extends l {
    ...
    set width(t) {
        super.width = t,
        (this.isUp || this.isDown) && this.element.setAttribute("width", 16 * Math.round(this.width / 16))
    }
    get height() {
        return super.height
    }
    set height(t) {
        super.height = t,
        (this.isLeft || this.isRight) && this.element.setAttribute("height", 16 * Math.round(this.height / 16))
    }
    get on() {
        return !!this._on
    }
...
class X extends l {
    constructor(t, e) {
        super(document.getElementById(t)),
        this.pressed = e
    }
    tick() {
...
class Y extends l {
    constructor(s) {
        ...
        this.game = s,
        this.keys = [new X("key-w",()=>t.has("w")), new X("key-a",()=>t.has("a")), new X("key-d",()=>t.has("d")), new X("key-space",()=>t.has(" ")), new X("button-toggle",()=>e.has(1)), new X("button-jump",()=>e.has(0)), new X("button-left",()=>e.has(14)), new X("button-right",()=>e.has(15))]
    }
    keydown({key: t}) {
        switch (t) {
        case "Enter":
...
const Q = 3
  ...
  , tt = Z.createSVGPoint()
  , et = ({clientX: t, clientY: e})=>(tt.x = t,
tt.y = e,
tt.matrixTransform(Z.getScreenCTM().inverse()));
let st = null
  , it = null;
...
class ht extends p {
    ...
    region(t, e) {
        return t -= this.x,
        e -= this.y,
        t <= Q ? e <= Q ? "nw" : e < this.height - Q ? "w" : "sw" : t < this.width - Q ? e <= Q ? "n" : e < this.height - Q ? "m" : "s" : e <= Q ? "ne" : e < this.height - Q ? "e" : "se"
    }
    cursor(t) {
        switch (t) {
        case "n":
...
class nt extends j {
    ...
    region(t, e) {
        return t -= this.x,
        e -= this.y,
        this.isUp || this.isDown ? t <= Q ? "w" : t < this.width - Q ? "m" : "e" : e <= Q ? "n" : e < this.height - Q ? "m" : "s"
    }
    cursor(t) {
        switch (t) {
        case "n":
...
class at extends l {
    ...
    addBar(t) {
        this.bars.push(t),
        this.append(t),
        t.element.addEventListener("click", ({shiftKey: e})=>{
            e && (t.remove(),
            this.bars = this.bars.filter(e=>e === t))
        }
        )
    }
    addSpike(t) {
        ...
        t.element.addEventListener("click", ({shiftKey: e})=>{
            e && (t.remove(),
            this.spikes = this.spikes.filter(e=>e === t))
        }
        )
    }
    get level() {
    ...
    set level(t) {
        ...
        for (this.guy.load(...e),
        this.goal.load(...s); this.bars.length; )
            this.bars.pop().remove();
        for (const t of i)
            this.addBar(new ht(...t));
        for (; this.spikes.length; )
            this.spikes.pop().remove();
        for (const t of h)
            this.addSpike(new nt(...t))
    }
    keydown({key: e}) {
        if (!this.hidden)
            switch (e) {
            ...
            case "g":
                ...
                s.searchParams.set("level", JSON.stringify(this)),
                window.location = s.toString();
                break;
            case "Escape":
...
class ct extends l {
    constructor(t, e) {
        ...
        this.deaths = new K(document.getElementById("death-counter")),
        this.stars = new K(document.getElementById("level-counter")),
        this.congrats = new l(document.getElementById("congrats")),
        this.esc = new l(document.getElementById("esc")),
        this.game = t,
        this.levels = e,
        this.bars = [],
        this.spikes = [],
    ...
    keydown({key: t}) {
        switch (t) {
        case "Enter":
            this.finished && (this.fromURL = !1,
            this.levels = c,
            this.game.state = "title",
            I());
            break;
        case "Escape":
            this.fromURL && (this.fromURL = !1,
            this.levels = c,
            this.game.state = "title",
            I())
        }
    ...
    set on(t) {
        ...
        document.body.classList.toggle("off", !t)
    }
    get index() {
        return this._index
    }
    set index(t) {
        for (this._index = Math.min(this.levels.length, Math.max(t || 0)),
        this.on = !0,
        this.stars.value = this.index; this.bars.length; )
            this.bars.pop().remove();
        for (; this.spikes.length; )
            this.spikes.pop().remove();
        if (this.finished)
            return this.guy.hidden = !0,
            this.congrats.hidden = !1,
            void T();
    ...
    async advance() {
        ...
        await d(1e3),
        this.finished ? this.goal.hidden = !0 : this.paused = !1
    }
    async death() {
        $.play(),
        this.deaths.value += 1,
        ...
        t.setAttribute("x", this.guy.x - 32 + this.guy.width / 2),
        t.setAttribute("y", this.guy.y - 32 + this.guy.height / 2),
        this.guy.element.setAttribute("hidden", !0),
    ...
    setBounds(t) {
        ...
        e.right = H - t.right,
        e.top = -t.top,
        e.bottom = V - t.bottom + 1;
        for (const s of this.bars)
            s.on === this.on && (s.top < t.bottom && s.bottom > t.top && (s.isRightOf(t) ? e.right = Math.min(e.right, s.left - t.right) : s.isLeftOf(t) && (e.left = Math.max(e.left, s.right - t.left))),
            s.left < t.right && s.right > t.left && (s.isBelow(t) ? e.bottom = Math.min(e.bottom, s.top - t.bottom) : s.isAbove(t) && (e.top = Math.max(e.top, s.bottom - t.top))));
        return e
    }
    tick(t) {
        if (this.paused || this.hidden)
            return;
        this.guy.tick(t);
        const {left: e, right: s} = this.setBounds(this.guy);
        this.guy.x += Math.min(s, Math.max(e, this.guy.vx));
        const {top: h, bottom: n} = this.setBounds(this.guy);
        this.guy.y += Math.min(n, Math.max(h, this.guy.vy)),
        0 === n ? (this.guy.vy = i() ? -t(1200) : 0,
        i() && O.play()) : this.guy.vy = Math.min(t(600), this.guy.vy + t(120)),
        this.lost() ? this.death() : this.guy.overlaps(this.goal) && this.advance()
    }
}
const dt = new class {
    constructor() {
        this.title = new v(this),
        this.controls = new Y(this),
        this.scene = new ct(this,c),
        this.editor = new at([[[100, 300], [500, 300], [[84, 361, 362, 48, 1]], [[446, 401, 176, 8, 1, "up"]]]],this),
        this.dialog = document.getElementById("dialog"),
        r(1, this.toggle.bind(this)),
        document.addEventListener("keydown", this.keydown.bind(this))
    }
    toggle() {
        this.scene.on = !this.scene.on,
        this.scene.on ? P.play() : J.play()
    }
    keydown(t) {
        " " === t.key && this.toggle(),
        this.scene.hidden ? this.controls.hidden ? this.title.hidden || this.title.keydown(t) : this.controls.keydown(t) : this.scene.keydown(t)
    }
    get state() {
        return this._state
    }
    set state(t) {
        this._state = t,
        this.scene.hidden = "play" !== this.state,
        this.title.hidden = "title" !== this.state,
        this.controls.hidden = "controls" !== this.state,
        this.editor.hidden = "edit" !== this.state,
        this.dialog.hidden = "edit" !== this.state
    }
    tick(t) {
        this.scene.tick(t),
        this.controls.tick(t)
    }
}
  , lt = new URL(window.location).searchParams.get("level");
if (lt)
    try {
        dt.scene.levels = [JSON.parse(lt)],
        dt.scene.fromURL = !0,
        dt.scene.index = 0,
        dt.state = "play"
    } catch (t) {}
